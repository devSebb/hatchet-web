...
function hashDir(h, dir, prefix, skip) {
  ...
  for (const e of entries.sort((a, b) => (a.name < b.name ? -1 : 1))) {
    ...
    if (e.isDirectory())
      hashDir(h, join(dir, e.name), `${prefix}${e.name}/`, skip);
...
export function renderHashFor(OUT, c, { stories, srcSha } = {}) {
  ...
  try {
    const html = readFileSync(
      join(OUT, "components", c.group, c.name, `${c.name}.html`),
      "utf8",
    );
  ...
  if (stories)
    h.update(
      JSON.stringify(
        stories.map((s) => [s.name, s.exportKey ?? null, s.emitted ?? null]),
      ),
    );
...
export function configSlicesFor(
  cfg = {},
  designSyncDir = resolve(".design-sync"),
) {
  ...
  let forks = [];
  try {
    forks = readdirSync(join(designSyncDir, "overrides"))
  ...
  for (const f of forks)
    hashFile(g, join(designSyncDir, "overrides", f), `fork:${f}`);
...
export function scriptsShaFor() {
  ...
  for (const f of [
    ...
    "package-capture.mjs",
    "resync.mjs",
    "storybook/compare.mjs",
    "storybook/http-serve.mjs",
    "storybook/probe.mjs",
  ]) {

...
const NAMES = (flag("components") ?? "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
if (!NODE_MODULES || !OUT || !NAMES.length) {
  console.error(
    "required: --node-modules --out --components A,B --config .design-sync/config.json (--config optional only for pre-sourceKey bundles)",
  );
  process.exit(1);
}
...
if (!existsSync(mapPath)) {
  console.error(
    `[NO_MANIFEST] ${mapPath} not found — run package-build.mjs first.`,
  );
  process.exit(1);
}
...
for (const n of NAMES) {
  ...
  if (!group) {
    console.error(
      `! ${n}: not in .stories-map.json and no components/*/${n}/ dir — skipped`,
    );
    continue;
  }
  ...
  if (
    ...
    console.error(
      `! ${n}: no ${n}.tsx in .design-sync/previews/ (owned) or .design-sync/.cache/previews/ (generated) — skipped`,
    );
    continue;
  }
  ...
  if (!owned)
    console.error(
      `! ${n}: rebuilding from the generated cache twin (.design-sync/.cache/previews/) — in-place edits there do NOT move the grade key; move the file to .design-sync/previews/${n}.tsx to take ownership (re-keys + re-grades)`,
    );
  targets.push({ name: n, group });
}
if (!targets.length) {
  console.error("[ZERO_MATCH] nothing to rebuild");
  process.exit(1);
}
...
if (
  manifest.keyRecipe === KEY_RECIPE &&
  manifest.cfgSliceGlobal !== undefined
) {
  ...
  if (!CONFIG_PATH) {
    console.error(
      "✗ this bundle carries stamped grade keys — pass --config .design-sync/config.json (the stamp guard compares live config against the build)",
    );
    process.exit(1);
  }
  const live = configSlicesFor(cfg);
  const stale =
    live.global !== manifest.cfgSliceGlobal
      ? "config or .design-sync/overrides/"
      : targets.some(
            (t) =>
              byName.get(t.name)?.cfgSlice !== undefined &&
              live.componentFor(t.name) !== byName.get(t.name).cfgSlice,
          )
        ? "cfg.overrides/cfg.titleMap for a target component"
        : null;
  if (stale) {
    console.error(
      `✗ [CONFIG_STALE] ${stale} changed since the stamped build — run package-build.mjs first (the full build re-stamps the grade keys)`,
    );
    process.exit(1);
  }
}
...
let tsconfigPath =
  cfg.tsconfig && PKG_DIR ? resolve(PKG_DIR, cfg.tsconfig) : null;
if (tsconfigPath) {
  try {
    const r = relative(
      gitWorkspaceRoot(realpathSync(dirname(NODE_MODULES))),
      realpathSync(tsconfigPath),
    );
    if (r.startsWith("..") || isAbsolute(r)) {
      console.error(
        `  ! tsconfig: ${cfg.tsconfig} resolves outside the workspace root — skipped`,
      );
      tsconfigPath = null;
    }
  } catch {
    tsconfigPath = null;
  } // missing/unreadable — same as absent
}
...
for (const t of targets) {
  if (!built.has(t.name)) {
    failed++;
    continue;
  } // buildPreviews already printed the esbuild error
  ...
  if (ov.cardMode && ov.cardMode !== "single" && ov.cardMode !== "column") {
    console.error(
      `  ! cfg.overrides.${t.name}.cardMode "${ov.cardMode}" isn't "single" or "column" — rendering as a plain grid`,
    );
  }
  const card =
    ov.cardMode === "single"
...
if (
  sidecar &&
  (!sidecar.renderHashes ||
    typeof sidecar.renderHashes !== "object" ||
    Array.isArray(sidecar.renderHashes))
) {
  console.error(
    "! _ds_sync.json malformed (renderHashes) — sidecar not patched; run a full package-build before validate/upload",
  );
  sidecar = null;
}
...
try {
  ...
  for (const t of targets) {
    ...
    if (
      live.keyRecipe !== KEY_RECIPE ||
      c.cfgSlice === undefined ||
      live.cfgSliceGlobal === undefined
    ) {
      delete c.sourceKey;
    ...
    c.sourceKey = sourceKeyFor(t.name, {
      ...
      ...(sbShape
        ? { stories: c.stories ?? [], srcSha: c.srcSha ?? null }
        : {}),
    });
...
if (sidecar) {
  try {
    for (const t of targets) {
      ...
      if (!c) {
        console.error(
          `! ${t.name}: not in the manifest — sidecar entry not patched; run a full package-build before validate/upload`,
        );
        continue;
      }
      sidecar.renderHashes[t.name] = renderHashFor(
        OUT,
      ...
      if (sidecar.sourceKeys) {
        ...
        if (sidecar.keyRecipe === KEY_RECIPE && restamped.has(t.name) && rk)
          sidecar.sourceKeys[t.name] = rk;
    ...
    console.error(
      `! _ds_sync.json not updated (${String(e.message ?? e).split("\n")[0]}) — run a full package-build before validate/upload`,
    );
  }
}

console.error(
  `✓ rebuilt ${built.size}/${targets.length} preview(s)${failed ? ` — ${failed} failed to compile (fix the .tsx and re-run)` : ""}`,
);

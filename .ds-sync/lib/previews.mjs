...
const MARKER_TAIL =
  "— generated; to OWN it, copy to .design-sync/previews/ and delete this line there.";
export const MARKER_RE =
  /^\uFEFF?\/\/ @ds-preview generated(?:\s+([0-9a-f]{12}))?\b/;
const bodyHash = (s) =>
  createHash("sha256").update(s).digest("hex").slice(0, 12);
const markerLine = (body) =>
  `// @ds-preview generated ${bodyHash(body)} ${MARKER_TAIL}`;
...
export function writePreviewFiles({ components, previewDir, genDir, gen }) {
  ...
  for (const c of components) {
    ...
    if (ownedTxt !== null) {
      ...
      if (twin !== null) {
        ...
        if (
          tm &&
          (!tm[1] || tm[1] === bodyHash(tnl < 0 ? "" : twin.slice(tnl + 1)))
        ) {
          rmSync(genPath);
        } else {
          console.error(
            `  (modified cache twin kept: ${c.name} — the owned .design-sync/previews/${c.name}.tsx wins; delete .design-sync/.cache/previews/${c.name}.tsx yourself if it is no longer wanted)`,
          );
        }
      }
      continue;
    ...
    if (genTxt !== null) {
      ...
      if (
        !m ||
        (m[1] && m[1] !== bodyHash(nl < 0 ? "" : genTxt.slice(nl + 1)))
      ) {
        modified++;
        console.error(
          `  (preview modified in the cache: ${c.name} — NOT regenerating over it; it is gitignored AND outside the grade key, so edits here never re-grade — move it to .design-sync/previews/${c.name}.tsx, minus any marker line, to own it durably and re-key it)`,
        );
        continue;
      }
    }
  ...
  if (markered.length) {
    ...
    console.error(
      `  (note: ${markered.length} owned preview(s) in .design-sync/previews/ still carry the generated marker on line 1 — delete the line; owned files are markerless: ${shown}${more})`,
    );
  }
  ...
  for (const f of readdirSync(genDir)) {
    ...
    try {
      txt = readFileSync(p, "utf8").replace(/\r\n/g, "\n");
    } catch {
      continue;
    }
    ...
    if (m && (!m[1] || m[1] === bodyHash(nl < 0 ? "" : txt.slice(nl + 1)))) {
      ...
      console.error(
        `  (stale preview kept: ${n} — component no longer exported; modified in the cache, never deleted)`,
      );
    }
  }
  const extras = [
    overrides && `${overrides} user-owned`,
    modified && `${modified} modified-in-cache`,
  ].filter(Boolean);
  console.error(
    `  previews: ${generated} generated → .design-sync/.cache/previews/${extras.length ? ` (${extras.join(", ")})` : ""}`,
  );
}
...
export async function buildPreviews({
  ...
  for (const c of components) {
    ...
    try {
      ...
      console.error(
        `  ! preview build failed: ${c.name}: ${firstLine}${where}`,
      );
      if (loc?.lineText)

...
export function gitWorkspaceRoot(base) {
  ...
  if (homeEnv) {
    try {
      home = realpathSync(homeEnv);
    } catch {
      home = resolve(homeEnv);
    }
  }
  let d = base;
  while (true) {
...
export function titleParts(title, titleMap = {}, exportedSet = null) {
  ...
  if (exportedSet) {
    for (let i = segs.length - 1; i >= 0; i--) {
      if (exportedSet.has(titleMap[segs[i]] ?? segs[i])) {
        idx = i;
        break;
      }
    }
  ...
  if (
    Object.prototype.hasOwnProperty.call(titleMap, name) &&
    titleMap[name] === null
  ) {
...
export function leadingJsdoc(text, name) {
  const declRx = name
    ? new RegExp(
        `(?:export\\s+)?(?:declare\\s+)?(?:const|let|function|class|interface|type)\\s+${name}\\b`,
      )
...
export const remedyFor = (t) =>
  ERROR_REMEDIES.find(([re]) => re.test(String(t ?? "")))?.[1] ?? "";
...
export const REMOVED_CONFIG_KEYS = new Map([
  [
    "previewArgs",
    "the generated-preview tier is gone — author .design-sync/previews/<Name>.tsx instead (it fully replaces previewArgs), then delete this key",
  ],
]);

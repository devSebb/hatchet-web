...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    define: {
      ...
      // Next.js NEXT_PUBLIC_* env vars — undefined in the design tool; all have
      // inline fallbacks in the source (e.g. ?? "https://hatchet.com") so IIFE
      // initialization succeeds and components render with sensible defaults.

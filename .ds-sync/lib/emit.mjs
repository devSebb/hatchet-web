...
export async function vendorReact({ nodeModules, out }) {
  ...
  if (existsSync(reactUmd)) {
    ...
    console.error(
      `  react@${reactPkg.version} has no UMD — bundling via esbuild`,
    );
    await build({
      stdin: {
        contents:
...
export function previewHtmlModule(
  ...
  const viewportAttr = card.viewport
    ? ` viewport="${escapeHtml(card.viewport)}"`
    : "";
...
export function providerWrapper(PROVIDER, GLOBAL, hasDecorators) {
  if (!PROVIDER && hasDecorators) {
    return (expr) =>
      `(window.__dsDecorate?window.__dsDecorate(${expr}):${expr})`;
  }
  ...
  const providerProps = (props, G) => {
    const pairs = Object.entries(props ?? {}).map(([k, v]) => {
      ...
      if (v && typeof v.$ref === "string") {
        ...
        console.error(
          `[PROVIDER_INVALID] $ref "${v.$ref}" isn't a valid identifier path`,
        );
...
function storySnippets(c, visibleStoryIds) {
  ...
  const parseFile = (p) => {
    ...
    const entry = {
      lines,
      starts,
      boundaries: [...starts.values()].sort((a, b) => a - b),
    };
    parsed.set(p, entry);
    return entry;
  };
...
export function emitPerComponent({
  ...
  for (const c of components) {
    ...
    const replaces = REPLACES[c.name]
      ? ` * @replaces ${REPLACES[c.name]}\n`
      : "";
    ...
    writeFileSync(
      join(dir, `${c.name}.d.ts`),
      dts.replace(/ \/\* @(?:fn|arr) \*\//g, ""),
    );
    ...
    const kw = c.docKeywords?.length
      ? ` Keywords: ${c.docKeywords.join(", ")}.`
      : "";
    ...
    if (c.docBody) {
      ...
      if (visibleStoryIds.length) {
        ...
        parts.push(
          `Variants (see \`${c.name}.html\`): ${variantNames.join(", ")}.\n`,
        );
      }
      ...
      exParts.push(
        ...previewExamples(existsSync(ownedTsx) ? ownedTsx : genTsx),
      );
      if (exParts.length)
        parts.push(`## Examples\n\n${exParts.join("\n\n")}\n`);
      // ## Related.
...
export function emitReadme({
  ...
  const hasBundleCss =
    bundleCssText.trim().length > 0 &&
    !bundleCssText.startsWith("/* @ds-css-runtime");
  ...
  for (const t of tokenNames) {
    const k = /color|bg-|fg-|text-|fill|border-(?!radius|width)|surface/i.test(
      t,
    )
      ? "color"
...
`;
  ...
  const assembled = headerText.trim()
    ? headerText.trimEnd() + "\n\n" + readme
    : readme;
...
export function emitBuildMeta({
  ...
  const skippedStoryIds = [
    ...new Set(Object.values(OVERRIDES).flatMap((o) => o?.skip ?? [])),
  ];

...
export function extractFonts(cssPath, srcDir, { fontsOut, roots }) {
  ...
  const rootsReal = (Array.isArray(roots) ? roots : [roots]).map(
    (r) => realOf(resolve(r)) ?? resolve(r),
  );
  const insideRoots = (p) =>
    rootsReal.some((root) => {
...
export function copyTokens({ tokensPkg, tokensGlob, nodeModules, out }) {
  ...
  console.error(
    `  tokens: ${tokenFiles.length} files from ${tokensPkg}@${tjson.version}`,
  );
  return tokenFiles;
}
...
export function rewriteBundleFontFaces({ out, bundleCss }) {
  ...
  const next = css.replace(/@font-face\s*\{[^}]*\}/gi, (block) => {
    ...
    for (const m of block.matchAll(/url\(\s*['"]?([^'")]+)['"]?\s*\)/gi)) {
      ...
      if (existsSync(join(out, "fonts", name))) {
        b = b.split(u).join(`./fonts/${name}`);
        rewrote++;
      }
    }
    if (/url\(\s*['"]?(?!https?:|data:|\.\/fonts\/)/i.test(b)) {
      dropped++;
      return "/* @ds-font-face-dropped: unresolvable src */";
    }
    return b;
  });
  if (rewrote || dropped) {
    ...
    console.error(
      `  _ds_bundle.css fonts: ${rewrote} url(s) rewritten to fonts/${dropped ? `, ${dropped} dead @font-face block(s) dropped` : ""}`,
    );
  }
}
...
export function writeStylesCss({
  out,
  tokenFiles,
  bundleCss,
  fontRules,
  ...
  try {
    ...
    hasBundleCss =
      css.trim().length > 0 && !css.startsWith("/* @ds-css-runtime");
  } catch {
    /* absent */
  }
  const styleImports = [
  ...
  if (styleImports.length) {
    ...
    console.error(
      `  styles.css: ${styleImports.length} @import(s)${hasBundleCss ? " (incl. _ds_bundle.css — component styles ship to designs via this closure)" : ""}`,
    );
    return;
  }
  writeFileSync(
  ...
  console.error(
    "[CSS_RUNTIME] no static CSS found (tokens/component/fonts/remote all empty) — wrote a self-styling styles.css. Expected for CSS-in-JS DSes; if this DS does ship a stylesheet, set cfg.cssEntry to it. If cfg.cssEntry is ALREADY set and renders verify, this line refers only to the scrape — do not chase it.",
  );
}

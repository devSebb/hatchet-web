...
export function findStorybookDirs(root, depth = 4) {
  ...
  (function walk(d, lvl) {
    ...
    for (const e of ls(d, { withFileTypes: true })) {
      ...
      if ((e.name === ".storybook" || e.name === "storybook") && isConfigDir(p))
        out.push(p);
      else walk(p, lvl + 1);
    }
  })(root, 0);
...
export function detectShape({ INPUTS, SB_STATIC, SB_CONFIG_DIR }) {
  if (SB_STATIC || SB_CONFIG_DIR) {
    console.error(
      `[DETECT] shape=storybook (explicit ${SB_STATIC ? "--storybook-static" : "--storybook-config"})`,
    );
    return "storybook";
  }
  ...
  console.error(
    `[DETECT] searched ${INPUTS} (depth 4), found .storybook at [${found.join(", ")}] → shape=${shape}`,
  );
  return shape;
}

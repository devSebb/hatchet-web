...
function findComponentDoc(c, { docsDirFiles, mapped, cfgPath }) {
  ...
  if (exact.length > 1) {
    console.error(
      `[DOCS_AMBIGUOUS] ${c.name}: ${exact.length} docs slug-match (${exact.map((p) => basename(p)).join(", ")}) — using ${basename(exact[0])}; pin cfg.docsMap.${c.name} to choose`,
    );
  }
...
export function discoverDocs({ components, PKG_DIR, cfg, cfgPath }) {
  ...
  console.error(
    `  docs: ${matched}/${components.length} components matched${docsDir ? ` (cfg.docsDir=${relative(PKG_DIR, docsDir) || "."})` : ""}${viaMap ? ` — ${viaMap} via docsMap, ${matched - viaMap} discovered` : ""}${excluded ? `, ${excluded} excluded (docsMap null)` : ""}`,
  );
  if (matched > 0)

...
};
{
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    console.error(
      `(unrecognized argument "${argv[i]}" — ignored; usage: remote-diff.mjs --local <ds-bundle> [--remote <saved-sidecar.json>])`,
    );
  }
}
...
if (!OUT) {
  console.error(
    "usage: remote-diff.mjs --local <ds-bundle dir> [--remote <saved-sidecar.json>]",
  );
  process.exit(2);
}
...
const validSidecar = (s) =>
  ...
  s.renderHashes &&
  typeof s.renderHashes === "object" &&
  !Array.isArray(s.renderHashes) &&
  s.sourceHashes &&
  typeof s.sourceHashes === "object" &&
  !Array.isArray(s.sourceHashes);

let local;
try {
  local = JSON.parse(readFileSync(join(OUT, "_ds_sync.json"), "utf8"));
} catch (e) {
  console.error(
    `✗ ${OUT}/_ds_sync.json unreadable (${e.message}) — run package-build.mjs first`,
  );
  process.exit(1);
}
if (!validSidecar(local)) {
  console.error(
    "✗ local _ds_sync.json malformed (styleSha/renderHashes/sourceHashes) — rebuild",
  );
  process.exit(1);
}
...
let liveBundleSha;
try {
  liveBundleSha = createHash("sha256")
    .update(readFileSync(join(OUT, "_ds_bundle.js")))
    .digest("hex")
    .slice(0, 12);
} catch {
  console.error(
    `✗ ${OUT}/_ds_bundle.js unreadable — run package-build.mjs first`,
  );
  process.exit(1);
}
if (local.bundleSha12 !== liveBundleSha) {
  console.error(
    "✗ local _ds_sync.json is stale (bundleSha mismatch with _ds_bundle.js) — rebuild before diffing",
  );
  process.exit(1);
}
...
if (remotePath) {
  ...
  try {
    ...
    console.error(
      `! remote sidecar unreadable (${e.message}) — treating as no anchor`,
    );
  }
  if (remote && !validSidecar(remote)) {
    ...
    console.error(
      `! source shape changed (${remote.shape} → ${local.shape}) — hashes are not comparable across recipes; full re-verification`,
    );
    remote = null;
...
if (!remote) {
  ...
  console.error(
    `no remote anchor — full scope (${localNames.length} component(s) verify + upload)`,
  );
} else {
  ...
  const keysOk = (s) =>
    s.sourceKeys &&
    typeof s.sourceKeys === "object" &&
    !Array.isArray(s.sourceKeys);
  const useSourceKeys =
    keysOk(local) &&
    keysOk(remote) &&
    local.keyRecipe !== undefined &&
    remote.keyRecipe === local.keyRecipe;
  if (keysOk(remote) && keysOk(local) && !useSourceKeys) {
    console.error(
      `! source-key recipe changed (remote keyRecipe ${remote.keyRecipe} → local ${local.keyRecipe}) — falling back to the render-hash partition (full re-verification of changed artifacts)`,
    );
  }
  ...
  for (const n of localNames) {
    if (!(n in remote.renderHashes)) {
      out.added.push(n);
      continue;
    }
    // A name missing from either sourceKeys map is unknown — re-verify.
    const changed = useSourceKeys
      ? remote.sourceKeys[n] === undefined ||
        local.sourceKeys[n] === undefined ||
        remote.sourceKeys[n] !== local.sourceKeys[n]
    ...
    if (useSourceKeys && remote.renderHashes[n] !== local.renderHashes[n])
      out.renderChurned.push(n);
  }
  out.removed = Object.keys(remote.renderHashes).filter(
    (n) => !(n in local.renderHashes),
  );
  ...
  for (const n of [...out.added, ...out.changed, ...out.renderChurned])
    uploadSet.add(n);
  ...
  for (const [name, info] of remoteBy) {
    ...
    out.upload.deletePaths.push(
      ...info.paths.filter((p) => !localPathSet.has(p)),
    );
  ...
  for (const n of Object.keys(remote.renderHashes)) {
    if (!(n in local.renderHashes) && !remoteBy.has(n)) {
      console.error(
        `! removed component "${n}" has no sourceHashes coverage in the remote sidecar — its remote files can't be derived for deletion; list_files and clean up by hand once`,
      );
    }
  }
  ...
  out.upload.aux =
    remote.auxSha === undefined ||
    local.auxSha === undefined ||
    remote.auxSha !== local.auxSha;
  out.upload.any =
    out.upload.components.length > 0 ||
    out.upload.deletePaths.length > 0 ||
    out.upload.bundle ||
    out.upload.styling ||
    out.upload.aux;
  ...
  if (
    ...
    keysOk(remote) &&
    !out.upload.any &&
    typeof local.keyRecipe === "number" &&
    typeof remote.keyRecipe === "number" &&
  ...
  console.error(
    `verify: ${out.unchanged.length} verified-by-upload (skip capture/grade), ${out.changed.length} changed, ${out.added.length} new, ${out.removed.length} removed${out.renderChurned.length ? `, ${out.renderChurned.length} artifact-churned with stable sources (grades kept — spot-check)` : ""} [keyed on ${out.keyedBy}]`,
  );
  console.error(
    out.upload.any
      ? out.upload.components.length ||
        out.upload.deletePaths.length ||
        out.upload.bundle ||
        out.upload.styling ||
        out.upload.aux

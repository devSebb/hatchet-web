...
if (!OUT || !existsSync(OUT)) {
  console.error(
    "usage: node package-validate.mjs <out-dir> [--render-sample N]",
  );
  process.exit(1);
}
...
const previewRemedy = (name) =>
  existsSync(join(".design-sync", "previews", `${name}.tsx`))
...
try {
  ...
  if (ver.dtsStubbed)
    fail(
      "[DTS_STUBBED] built with --skip-dts — re-run package-build without it before the upload gate",
    );
} catch (e) {
  fail(`.ds-build-meta.json: ${e.message}`);
}
...
if (!existsSync(bundleJs))
  fail("_ds_bundle.js missing — [NO_DIST] the package build failed");
else {
  const src = readFileSync(bundleJs, "utf8");
  const kb = (statSync(bundleJs).size / 1024).toFixed(0);
  try {
    new Function(src);
    ok(`_ds_bundle.js: ${kb} KB, syntax OK`);
  } catch (e) {
    fail(`_ds_bundle.js: syntax error — ${e.message}`);
  }
  // Header: first line only, un-escape `*\/`.
  const m = /^\/\* @ds-bundle: (.*) \*\//.exec(src.split("\n", 1)[0]);
  if (!m)
    fail("_ds_bundle.js: missing first-line `/* @ds-bundle: {…} */` header");
  else {
    try {
      ...
      if (missing.length)
        fail(`_ds_bundle.js header missing field(s): ${missing.join(", ")}`);
      else if (
        typeof meta.namespace !== "string" ||
        !Array.isArray(meta.components)
      ) {
        fail(
          "_ds_bundle.js header: namespace must be a string and components an array",
        );
      } else {
        ok(
          `_ds_bundle.js header: window.${meta.namespace}, ${meta.components.length} components, ${meta.inlinedExternals.length} inlined externals`,
        );
        headerMeta = meta;
      }
    } catch (e) {
      fail(`_ds_bundle.js header: invalid JSON — ${e.message}`);
    }
  }
}
...
try {
  ...
  const n = badShape(sync.renderHashes)
    ? -1
    : Object.keys(sync.renderHashes).length;
  // n === 0 is legitimate for a tokens-only sync (componentCount 0).
  if (!sync.styleSha || n < 0 || (n === 0 && ver?.componentCount !== 0))
    fail("_ds_sync.json missing styleSha/renderHashes — rebuild");
  else {
    let live = null;
    try {
      live = createHash("sha256")
        .update(readFileSync(bundleJs))
        .digest("hex")
        .slice(0, 12);
    } catch {
      /* bundle missing — NO_DIST already failed above */
    }
    if (live && sync.bundleSha12 !== live)
      fail(
        "_ds_sync.json is stale (bundleSha mismatch) — rebuild so the anchor describes this bundle",
      );
  ...
  try {
    ...
    try {
      manifest = JSON.parse(
        readFileSync(join(OUT, ".stories-map.json"), "utf8"),
      );
    } catch {
      ok(
        "(render-hash recompute skipped — no .stories-map.json; off-script layouts skip this check)",
      );
    }
    const { renderHashFor } = await import(
      new URL("./lib/sync-hashes.mjs", import.meta.url).href
    );
    const stale = [];
    if (manifest) {
      for (const c of manifest.components ?? []) {
        const liveHash = renderHashFor(
          OUT,
          c,
          sync.shape === "storybook"
            ? {
                stories: (c.stories ?? []).map((st) => ({
                  name: st.name,
                  exportKey: st.exportKey ?? null,
                  emitted: st.emitted ?? null,
                })),
      ...
      if (stale.length)
        fail(
          `[SYNC_STALE] _ds_sync.json renderHashes don't match disk for: ${stale.join(", ")} — rebuild (package-build.mjs) so the anchor describes this output`,
        );
      else if (manifest.components?.length)
        ok(
          `_ds_sync.json render hashes match disk (${manifest.components.length} recomputed)`,
        );
    }
  } catch (e) {
    fail(
      `_ds_sync.json recompute failed (${String(e.message ?? e).split("\n")[0]})`,
    );
  }
} catch (e) {
  ...
  if (e?.code === "ENOENT" && !existsSync(join(OUT, ".stories-map.json")))
    warn(
      "_ds_sync.json absent — acceptable only for an off-script layout; with no anchor the next sync re-verifies everything",
    );
  else
    fail(
      `_ds_sync.json unreadable (${e.message}) — the verification anchor must upload with the bundle`,
    );
}
...
if (!existsSync(stylesCss))
  fail("styles.css missing — the styles entry point the app reads");
else {
  ...
  for (const m of txt.matchAll(/@import\s+(?:url\()?["']([^"']+)["']/g)) {
    ...
    if (!existsSync(join(OUT, m[1]))) {
      missing++;
      fail(
        `[CSS_IMPORT_MISSING] styles.css @imports "${m[1]}" which doesn't exist under ${OUT}`,
      );
    }
  }
  if (n > 0) {
    if (!missing) ok(`styles.css: ${n} @import(s), all resolve`);
  } else if (/@ds-styles:\s*runtime/.test(txt)) {
    warn(
      "[CSS_RUNTIME] styles.css has no @imports — DS styles itself at runtime (CSS-in-JS). OK; verify the render check passes. If the DS does ship a stylesheet, set cfg.cssEntry. Already set cfg.cssEntry and renders verify? Then this is informational — do not chase it.",
    );
  } else {
    fail(
      "styles.css has no @import lines — no tokens/component/font CSS was scraped",
    );
  }
  ...
  let bundleTxt = "";
  try {
    bundleTxt = readFileSync(join(OUT, "_ds_bundle.css"), "utf8");
  } catch {
    /* CSS-in-JS / headless */
  }
  if (
    bundleTxt.trim() &&
    !bundleTxt.startsWith("/* @ds-css-runtime") &&
    !/@import\s+(?:url\()?["']\.\/_ds_bundle\.css["']/.test(txt)
  ) {
    fail(
      '[CSS_BUNDLE_UNREACHABLE] _ds_bundle.css has real CSS but styles.css does not @import it — rebuild (or add `@import "./_ds_bundle.css";`)',
    );
  }
  ...
  for (const m of bundleTxt.matchAll(/@import\s+(?:url\()?["']([^"']+)["']/g)) {
    ...
    if (!existsSync(join(OUT, m[1])))
      fail(
        `[CSS_IMPORT_MISSING] _ds_bundle.css @imports "${m[1]}" which doesn't exist under ${OUT}`,
      );
  }
}
...
if (existsSync(bundleCss)) {
  ...
  if (txt.includes("@ds-css-runtime")) {
    console.error(
      "[CSS_RUNTIME] _ds_bundle.css is the runtime-styles stub — expected for CSS-in-JS DSes",
    );
  } else if (sz < 500 && stripped.length === 0) {
    fail(
      `[CSS_PLACEHOLDER] _ds_bundle.css is ${sz}B of @import-only stub — set cfg.cssEntry to the compiled stylesheet (storybook repos: the build-time CSS fallback should have caught this — check for [CSS_FROM_STORYBOOK] in the build log)`,
    );
...
try {
  ...
  if (existsSync(stylesCss)) {
    for (const m of readFileSync(stylesCss, "utf8").matchAll(
      /@import\s+(?:url\()?["']([^"')]+)["']/g,
    )) {
  ...
  if (existsSync(bundleJs)) {
    ...
    for (const m of js.matchAll(/setProperty\(\s*['"`](--[\w-]+)/g))
      allCss += `\n${m[1]}:;`;
  ...
  const defined = new Set(
    [...allCss.matchAll(/(--[\w-]+)\s*:/g)].map((m) => m[1]),
  );
  const referenced = [
    ...new Set(
      [...allCss.matchAll(/var\(\s*(--[\w-]+)\s*\)/g)].map((m) => m[1]),
    ),
  ];
  ...
  if (missing.length > 3) {
    ...
    ok(
      `tokens: ${defined.size} defined, ${referenced.length} referenced${missing.length ? ` (${missing.length} missing, below threshold)` : ""}`,
    );
  }
} catch {}
...
try {
  ...
  const runtimePrefixes = (ver?.runtimeFontPrefixes ?? [])
    .map((p) => p.toLowerCase())
    .filter(Boolean);
  const isGeneric = (f) =>
  ...
  for (const p of cssPaths) {
    ...
    for (const m of chunk.matchAll(/@font-face\s*\{([^}]+)\}/g)) {
      ...
      let hasLocal = false,
        hasResolved = dangling.has(key) ? false : declared.has(key);
      for (const u of m[1].matchAll(
        /url\(\s*['"]?([^'")]+?\.(?:woff2?|ttf|otf|eot))(?:[?#][^'")]*)?['"]?\s*\)/gi,
      )) {
        if (/^(https?|data):/i.test(u[1])) {
          hasResolved = true;
          continue;
        }
        hasLocal = true;
  ...
  const hasRemoteFonts =
    /@import[^;]*(?:fonts\.googleapis|fonts\.gstatic|use\.typekit|fonts\.bunny)/i.test(
      css,
    );
  ...
  const resolveVars = (v, depth = 0) =>
    depth > 3
      ? v
      : v.replace(/var\(\s*(--[\w-]+)[^)]*\)/g, (_, name) =>
          customProps.has(name)
            ? resolveVars(customProps.get(name), depth + 1)
            : "",
        );
  ...
  const collect = (value, hint) => {
    for (let part of resolveVars(String(value)).split(",")) {
      part = part
        ...
        .replace(/^['"]|['"]$/g, "")
        .trim();
      if (!part || !/^\p{L}/u.test(part) || !/^[\p{L}\p{N}_ -]+$/u.test(part))
        continue;
  ...
  for (const [name, value] of customProps) {
    if (
      /font/i.test(name) &&
      !/font-(feature|variation|variant|kerning|stretch|optical|smooth(?:ing)?|size|weight|style|display|color|palette|leading|numeric|case|transform|synthesis)/i.test(
        name,
      )
    )
      collect(value, name);
  ...
  if (missing.size) {
    const list = [...missing.values()]
      .map((m) => `"${m.display}"${m.hint ? ` (${m.hint})` : ""}`)
      .join(", ");
    if (hasRemoteFonts) {
      ok(
        `[FONT_REMOTE] ${list} — a remote font-host @import is present; assuming it serves these at runtime`,
      );
    } else {
      warn(
        `[FONT_MISSING] ${list} referenced by the shipped CSS but no @font-face ships them — add the woff2 + @font-face via cfg.extraFonts, or accept substitutes (the DS pane will render with system fonts)`,
      );
    }
  }
  if (dangling.size) {
    const list = [...dangling.entries()]
      ...
      .join(", ");
    warn(
      `[FONT_DANGLING] ${list} — @font-face is shipped but its url() target isn't (the rule emits but the font file wasn't copied; check cfg.extraFonts paths or the build log for a "resolves outside" skip)`,
    );
  }
} catch {
  /* heuristic only — never block validation on a font-parse hiccup */
}
...
(function walk(d) {
  ...
  for (const e of readdirSync(d, { withFileTypes: true })) {
    ...
    if (e.name.endsWith(".html")) {
      ...
      if (
        !/^<!--\s*@dsCard\s+group="[^"]*"[^>]*-->/.test(txt.split("\n", 1)[0])
      ) {
        badCard++;
        fail(
          `[DSCARD_MISSING] ${rel}: first line isn't a \`<!-- @dsCard group="…" -->\` comment`,
        );
      }
      for (const m of txt.matchAll(/<link\b[^>]*\bhref="([^"]+)"/g)) {
        ...
        if (m[1].endsWith("/_ds_bundle.css") && !existsSync(bundleCss))
          continue;
        if (!existsSync(resolve(dirname(p), m[1]))) {
          badLink++;
          fail(
            `[LINK_HREF_MISSING] ${rel}: <link href="${m[1]}"> doesn't resolve`,
          );
        }
      }
      ...
      if (!readFileSync(p, "utf8").split("\n", 1)[0].trim()) {
        badPrompt++;
        fail(`[PROMPT_EMPTY] ${rel}: first line is empty`);
      }
    }
  }
})(join(OUT, "components"));
const tokensOnly = ver?.componentCount === 0;
if (previews === 0 && !tokensOnly)
  fail("no <Name>.html previews under components/");
else if (!badCard && !badLink && !badPrompt)
  ok(
    tokensOnly
      ? "tokens-only DS — no component previews"
      : `components/: ${previews} previews, ${prompts} .prompt.md`,
  );
if (ver && previews !== ver.componentCount) {
  fail(
    `count mismatch: ${previews} previews vs ${ver.componentCount} components`,
  );
}
...
try {
  ...
  (function walkDts(d) {
    for (const e of readdirSync(d, { withFileTypes: true })) {
      ...
      for (const diag of sf.parseDiagnostics ?? []) {
        ...
        fail(
          `[DTS_PARSE] ${relOut(p)}:${line + 1}: ${ts.flattenDiagnosticMessageText(diag.messageText, " ")}`,
        );
        dtsErrs++;
      }
    }
  ...
  console.error(
    "  (.d.ts parse check skipped — typescript not in node_modules)",
  );
}
...
if (!pw) {
  ...
  if (NO_RENDER_CHECK) {
    warn(
      "[RENDER_SKIPPED] render check did not run (--no-render-check) — previews are NOT visually verified",
    );
  } else {
    fail(
      "[RENDER_SKIPPED] playwright not importable — the render check did NOT run. `npm i -D playwright && npx playwright install chromium`, then re-run validate (or pass --no-render-check to accept an unverified bundle).",
    );
  }
} else {
  const htmls = [];
  ...
  const sample =
    RENDER_SAMPLE && htmls.length > RENDER_SAMPLE
      ? htmls.filter(
          (_, i) => i % Math.ceil(htmls.length / RENDER_SAMPLE) === 0,
        )
      : htmls;
  if (sample.length < htmls.length) {
    console.error(
      `  render check: sampling ${sample.length}/${htmls.length} previews (pass --render-sample 0 for all)`,
    );
  }
  const { serveDir } = await import(
    new URL("./storybook/http-serve.mjs", import.meta.url).href
  );
  ...
  try {
    browser = await pw.chromium.launch(
      process.env.DS_CHROMIUM_PATH
        ? { executablePath: process.env.DS_CHROMIUM_PATH }
        : {},
    );
    ...
    if (headerMeta?.components?.length) {
      try {
        ...
        await page
          .waitForFunction((g) => !!window[g], headerMeta.namespace, {
            timeout: 10_000,
          })
          .catch(() => {});
        const { exp, compound, bad } = await page.evaluate(
          ({ g, ns }) => {
            const NS = window[g] ?? {};
            const isFn = (v) => typeof v === "function" || (v && v.$$typeof);
            const isCompound = (v) =>
              v && typeof v === "object" && Object.values(v).some(isFn);
            const compound = [],
              bad = [];
            for (const n of ns) {
          ...
          {
            g: headerMeta.namespace,
            ns: headerMeta.components.map((c) => c.name),
          },
        );
        if (compound.length)
          console.error(
            `  [BUNDLE_EXPORT] ${compound.length} compound namespace(s) (usable via .Sub): ${compound.slice(0, 8).join(", ")}${compound.length > 8 ? ", …" : ""}`,
          );
        if (bad.length)
          fail(
            `[BUNDLE_EXPORT] ${bad.length}/${headerMeta.components.length} not a component on window.${headerMeta.namespace}: ${bad.slice(0, 8).join(", ")}${bad.length > 8 ? ", …" : ""}`,
          );
        else
          ok(
            `window.${headerMeta.namespace}: ${exp} exports (${headerMeta.components.length - compound.length} fn + ${compound.length} compound)`,
          );
      } catch (e) {
        console.error(
          `  (bundle-export smoke skipped — ${String(e).split("\n")[0]})`,
        );
      }
    }
    ...
    for (const rel of sample) {
        ...
        ({
          ...
          const paints = (root) => {
            for (const el of [root, ...root.querySelectorAll("*")]) {
              ...
              if (
                /^(IMG|SVG|CANVAS|VIDEO|IFRAME|PICTURE|HR)$/i.test(el.tagName)
              )
                return true;
          ...
          const portals = [...document.body.children].filter(
            (c) =>
              !c.matches(
                '#root, [id^="r"], .ds-grid, .ds-cell, section, script, style, link',
              ),
          );
          let caught = 0,
            firstCaught = null,
          ...
          const nPlaceholder = document.querySelectorAll(
            "[data-ds-placeholder]",
          ).length;
          const texts = [],
            htmls = [],
          ...
          for (const p of portals)
            maxHeight = Math.max(maxHeight, p.getBoundingClientRect().height);
          if (pText || pPaints)
          ...
          const variantsIdentical =
            htmls.length > 1 && htmls.every((h) => h === htmls[0]);
          const nFallback =
            document.querySelectorAll("[data-ds-fallback]").length;
          ...
          if (window.__dsMode === "grid" || window.__dsMode === "column") {
            ...
            const subtreeShows = (root) => {
              ...
              for (const el of [root, ...root.querySelectorAll("*")]) {
                ...
                if (
                  [...el.childNodes].some(
                    (t) => t.nodeType === 3 && t.textContent.trim(),
                  )
                )
                  return true;
            ...
            for (const cell of document.querySelectorAll("section.ds-cell")) {
              ...
              if (kind)
                gridOverflow =
                  kind === "escape" || gridOverflow === "escape"
                    ? "escape"
                    : "wide";
            }
            ...
            if (gridOverflow !== "escape" && portals.some(subtreeShows))
              gridOverflow = "escape";
          ...
          const storyExports = Array.isArray(window.__dsCells)
            ? window.__dsCells.slice(0, 8)
            : [];
          return {
            rootEmpty: !roots[0]?.innerHTML?.trim().length && !portals.length,
            caught,
            firstCaught,
            texts,
            nEls,
      ...
      const nameReps = (t) => {
        const s = squash(t);
        return s.length > 0 &&
          s.length % nameS.length === 0 &&
          s === nameS.repeat(s.length / nameS.length)
          ? s.length / nameS.length
          : 0;
      };
      ...
      results.push({
        ...
        errs,
        caught,
        firstErr: pageErrs[0] ?? firstCaught ?? err,
        pngBytes,
        blank,
        rootEmpty,
        thin: thin && !fallbackCard,
        nameOnly,
        allHollow,
        collapsed,
        hasPlaceholder,
        ...
        bad,
        gridOverflow,
        gridOverflowCells: gridOverflow ? gridOverflowCells : undefined,
        suggestedOverride,
        texts,
      });
      if (err) fail(`[RENDER] ${rel}: ${err}`);
        ...
        /* intentional floor — counted in the summary line */
      } else if (errs) {
        const first = pageErrs[0] ?? firstCaught;
        warn(
          `[RENDER_ERRORS] ${rel}: ${first} (${errs} total${caught ? `, ${caught} caught in-cell` : ""})`,
        );
        const hyp = hypothesisLine(first);
        if (hyp) console.error(hyp);
      } else if (blank)
        warn(
          `[RENDER_BLANK] ${rel}: renders but PNG is ${pngBytes}B (<5KB — likely blank; ${previewRemedy(name)})`,
        );
      else if (thin || variantsIdentical)
        warn(
          `[RENDER_THIN] ${rel}: ${variantsIdentical ? "variants render identically" : nameOnly ? `mounted text is just "${name}"` : collapsed ? `DOM content present but rendered height is ${Math.round(maxHeight)}px` : "mounts have no text and paint nothing"} — ${previewRemedy(name)}`,
        );
    ...
    writeFileSync(
      join(OUT, ".render-check.json"),
      JSON.stringify(results, null, 2),
    );
    const badOnes = results.filter((r) => r.bad);
    const floorOnes = results.filter((r) => r.fallbackCard);
    const floorNote = floorOnes.length
      ? ` (${floorOnes.length} showing the typographic floor card — unauthored previews, not failures)`
      : "";
    if (!badOnes.length)
      ok(
        `render check: ${results.length}/${results.length} previews render cleanly${floorNote} (screenshots in _screenshots/)`,
      );
    else
      console.error(
        `  render check: ${results.length - badOnes.length}/${results.length} clean; ${badOnes.length} need attention${floorNote} (see .render-check.json, _screenshots/)`,
      );
    ...
    try {
      ...
      if (results.length) {
        const PER_SHEET = 16;
        const entries = [...results].sort((a, b) =>
                ...
                : r.bad
                  ...
                  : r.thin
                    ...
                    : r.variantsIdentical
                      ...
                      : "✓";
        const borderOf = (r) =>
          r.bad ? "#d33" : r.thin || r.variantsIdentical ? "#d90" : "#ddd";
        const index = [];
        let failedTiles = 0;
        ...
        for (let s = 0; s < sheetCount; s++) {
          ...
          const cells = slice
            .map((r) => {
              ...
              let hasShot = false;
              try {
                hasShot = statSync(join(shotDir, shotName)).size > 0;
              } catch {
                hasShot = false;
              }
              const img = hasShot
              ...
              return (
                `<div style="border:2px solid ${borderOf(r)};background:#fff;min-width:0">` +
            ...
            .join("\n");
          const html =
            `<!doctype html><html><head><meta charset="utf-8"></head><body style="margin:0;background:#fff;width:1500px">` +
          ...
          await page.goto(
            `http://127.0.0.1:${port}/_screenshots/.contact-sheet-${s + 1}.html`,
          ...
          failedTiles += await page.evaluate(() => {
            ...
            for (const img of [...document.images]) {
              ...
              d.style.cssText =
                "width:330px;height:300px;display:flex;align-items:center;justify-content:center;color:#c00;font:14px system-ui";
          ...
          await page.screenshot({
            path: join(shotDir, `contact-sheet-${s + 1}.png`),
            fullPage: true,
          });
        ...
        for (const f of readdirSync(shotDir)) {
          ...
          if (m && Number(m[1]) > sheetCount)
            rmSync(join(shotDir, f), { force: true });
        }
        writeFileSync(
          join(shotDir, "contact-sheets.json"),
          JSON.stringify(index, null, 2),
        );
        console.error(
          `  contact sheets: ${sheetCount} sheet(s)${failedTiles ? `, ${failedTiles} tile(s) failed to load` : ""} → _screenshots/contact-sheet-1.png${sheetCount > 1 ? ` … contact-sheet-${sheetCount}.png` : ""}`,
        );
      }
    } catch (e) {
...
const warnNote = warnings
  ? ` (${warnings} warning(s) — review above, non-blocking)`
  : "";
console.error(
  errors

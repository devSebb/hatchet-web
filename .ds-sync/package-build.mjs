...
const forkedLibs = new Set(
  existsSync(REPO_LIB)
    ? readdirSync(REPO_LIB).filter((f) => f.endsWith(".mjs"))
    : [],
);
if (forkedLibs.has("sync-hashes.mjs")) {
  console.error(
    "[OVERRIDE_FORBIDDEN] sync-hashes.mjs cannot be forked — the sidecar, the grading harnesses, and remote-diff must share one recipe or carry-forward becomes unsound",
  );
  process.exit(1);
}
if (forkedLibs.has("preview-gen-package.mjs")) {
  console.error(
    "[OVERRIDE_DEAD] .design-sync/overrides/preview-gen-package.mjs is never loaded — the package-shape generated-preview tier is gone. Author .design-sync/previews/<Name>.tsx instead; delete the fork (and its cfg.libOverrides entry). NOTE: any fork add/delete moves the grade contract for every component — pair the deletion with a full build and expect a one-time full re-verify on the next sync.",
  );
}
...
const {
  ...
  stampHeader,
  tsconfigPathsPlugin,
} = await loadLib("bundle");
const { copyTokens, extractFonts, rewriteBundleFontFaces, writeStylesCss } =
  await loadLib("css");
const {
  exportedNames,
  findTypesRoot,
...
const { bundlePreviewDecorators, resolveStorybook } =
  await loadLib("source-storybook");
...
if (CONFIG_PATH) {
  try {
    ...
    console.error(`[CONFIG] ${CONFIG_PATH}: ${e.message}`);
    process.exit(1);
  }
  ...
  if (cfgErrors.length) {
    ...
    console.error(
      `[CONFIG] ${CONFIG_PATH}: ${cfgErrors.length} error(s) — fix the config and re-run`,
    );
    process.exit(1);
  }
}
...
for (const f of Object.keys(LIB_OVERRIDES)) {
  if (!forkedLibs.has(f))
    console.error(
      `[OVERRIDE_MISSING] cfg.libOverrides declares "${f}" but .design-sync/overrides/${f} doesn't exist`,
    );
}
...
if (ENTRY_OVERRIDE) {
  ...
  while (d !== dirname(d)) {
    try {
      ...
      /* missing or unparsable — keep walking */
    }
    d = dirname(d);
  }
...
const VERSION = /^\d+\.\d+\.\d+[\w.+-]*$/.test(pkgJson.version ?? "")
  ? pkgJson.version
  : "0.0.0";
// Generic pkg names (e.g. "app") → prefer the DS dir's own name.
const GENERIC_PKG = new Set([
  "app",
  "root",
  "frontend",
  "web",
...
const pkgNameForNs = GENERIC_PKG.has((pkgJson.name ?? "").toLowerCase())
  ? basename(PKG_DIR)
  : pkgJson.name;
...
// bundle (no _ds_bundle.js and no .ds-bundle marker). --out is user-supplied.
{
  ...
  const unsafe =
    ...
    (existsSync(outAbs) &&
      statSync(outAbs).isDirectory() &&
      !existsSync(join(outAbs, "_ds_bundle.js")) &&
      !existsSync(join(outAbs, ".ds-bundle")) &&
      readdirSync(outAbs).length > 0) ||
    (existsSync(outAbs) && !statSync(outAbs).isDirectory());
  if (unsafe) {
    console.error(
      `[OUT_UNSAFE] refusing to rm ${outAbs} — point --out at an empty dir or a prior bundle (never inside .design-sync/)`,
    );
    process.exit(1);
  }
}
...
if (shape !== "storybook" && shape !== "package") {
  console.error(
    `[CONFIG] cfg.shape must be 'storybook' or 'package', got ${JSON.stringify(cfg.shape)}`,
  );
  process.exit(1);
}
console.error(
  `  source shape: ${shape}${cfg.shape ? " (from cfg.shape)" : ""}`,
);
if (cfg.shape && cfg.shape !== autodetected)
  console.error(
    `[CONFIG] cfg.shape=${cfg.shape} overrides auto-detected '${autodetected}'`,
  );
...
const iconSiblings = depNames.filter((d) => {
  ...
  return dScope
    ...
    .replace(/[^a-z0-9]/gi, "")
    .toLowerCase()
    .startsWith(pkgSquash);
});
const extraEntries = [
  ...new Set([...(cfg.extraEntries ?? []), ...iconSiblings]),
];
...
function cfgPath(rel, field, root) {
  ...
  if (!existsSync(p)) {
    console.error(`  ! ${field}: ${rel} not found — skipped`);
    return undefined;
  }
  if (outside(realpathSync(p), root)) {
    console.error(
      `  ! ${field}: ${rel} resolves outside ${root === pkgRoot ? "the package" : "the workspace root"} — skipped`,
    );
    return undefined;
  }
...
if (extraEntries.length) {
  for (const p of iconSiblings)
    console.error(`  [ICON_PKG] auto-including sibling icon package ${p}`);
...
const exportEvidence = PROVIDER
  ? await bundleExportEvidence({
      entry: bundleEntry,
      nodePaths: NODE_MODULES,
      tsconfig: TSCONFIG_PATH,
    })
...
if (PROVIDER)
  console.error("  (decorator auto-detect skipped — cfg.provider is set)");
else if (!src.sbDir)
  console.error("  (decorator auto-detect skipped — no .storybook/ dir found)");
else
  hasDecorators = await bundlePreviewDecorators({
    sbDir: src.sbDir,
    OUT,
    NODE_MODULES,
    PKG,
...
if (explicitCss && existsSync(bundleCss)) {
  ...
  console.error(
    `  css: ${relative(INPUTS, explicitCss)} (${(statSync(explicitCss).size / 1024).toFixed(0)} KB, appended — bundle already had CSS)`,
  );
  ...
  const cand = explicitCss
    ...
    : src.cssEntry
      ...
      : [
          ...
          "dist/style.css",
          "styles.css",
        ].map((c) => join(PKG_DIR, c));
  for (const p of cand) {
    if (existsSync(p)) {
      cpSync(p, bundleCss);
      bundleCssSrcDir = dirname(p);
      console.error(
        `  css: ${relative(INPUTS, p)} (${(statSync(p).size / 1024).toFixed(0)} KB, copied)`,
      );
      break;
    }
  }
...
let sbFallback = null,
  remoteStyleImports = [];
if (src.sbStatic) {
  const { fallbackCssFromStorybook, scrapeRemoteImports } =
    await loadLib("css-fallback");
  sbFallback = fallbackCssFromStorybook({
    bundleCss,
    sbStatic: src.sbStatic,
    out: OUT,
  });
...
const fontRules = [
  ...
  ...(explicitCss
    ? extractFonts(explicitCss, dirname(explicitCss), {
        fontsOut,
        roots: PKG_DIR,
      })
    : []),
...
for (const rel of (typeof cfg.extraFonts === "string"
  ? [cfg.extraFonts]
  : cfg.extraFonts) ?? []) {
  ...
  if (/\.css$/i.test(p)) {
    fontRules.push(
      ...extractFonts(pReal, dirname(pReal), {
        fontsOut,
        roots: workspaceRoot,
      }),
    );
    ...
    console.error(
      `  extraFonts: copied ${basename(p)} — add a matching @font-face (e.g. an extraFonts .css) to use it`,
    );
  } else {
...
if (fontRules.length) {
  mkdirSync(fontsOut, { recursive: true });
  writeFileSync(
    join(fontsOut, "fonts.css"),
    [...new Set(fontRules)].join("\n") + "\n",
  );
...
if (!tokensPkg) {
  const tokenSibling = depNames.find((d) => {
    if (d === PKG || !/(?:^|[\/-])(?:tokens?|themes?)(?:$|[\/-])/i.test(d))
      return false;
    if (!existsSync(join(NODE_MODULES, d, "package.json"))) return false;
    if (scope && d.startsWith(scope + "/")) return true;
  ...
  if (tokenSibling) {
    ...
    console.error(
      `  [TOKENS_PKG] auto-detected sibling tokens package ${tokenSibling} (override with cfg.tokensPkg)`,
    );
  }
}
let tokenFiles = copyTokens({
  tokensPkg,
  tokensGlob: TOKENS_GLOB,
...
if (!tokenFiles.length && src.tokensCss?.length) {
  ...
  if (tokenFiles.length)
    console.error(
      `  tokens: ${tokenFiles.length} file(s) from source shape default`,
    );
}
...
for (const ep of extraEntries) {
  ...
  if (ep.startsWith("./") || ep.startsWith("../") || isAbsolute(ep)) {
    ...
    try {
        ...
        for (const m of src2.matchAll(
          ...
          try {
            ...
            if (!hop || outside(realpathSync(hop), workspaceRoot)) {
              console.error(
                `  ! extraEntries: star hop ${target} in ${ep} skipped (unresolvable or outside the workspace) — its names are unknown to the export gates`,
              );
              exportScanLossy = true;
              continue;
            }
            ...
            console.error(
              `  ! extraEntries: star hop ${target} in ${ep} failed (${String(e.message ?? e).split("\n")[0]}) — its names are unknown to the export gates`,
            );
            exportScanLossy = true;
          }
        }
      ...
      if (collisions.length) {
        console.error(
          `! [EXPORT_COLLISION] ${ep} exports ${collisions.length} name(s) the main package also exports: ${collisions.slice(0, 6).join(", ")}${collisions.length > 6 ? ", …" : ""} — stories importing these from ${ep} render the main package's binding. Fix: rename the export in ${ep}.`,
        );
      }
      ...
      console.error(
        `  ! extraEntries: ${ep} export scan failed (${String(e.message ?? e).split("\n")[0]}) — skipped for the export gates`,
      );
      exportScanLossy = true;
    }
    continue;
  ...
  try {
    ...
    if (collisions.length) {
      console.error(
        `! [EXPORT_COLLISION] ${ep} exports ${collisions.length} name(s) the main package also exports: ${collisions.slice(0, 6).join(", ")}${collisions.length > 6 ? ", …" : ""} — stories importing these from ${ep} render the main package's binding. Fix: cfg.storyImports.bundle: ["${ep}"] (bundle the sibling from source).`,
      );
    }
    for (const n of names) exported.add(n);
  } catch {
...
console.error(
  `  exported PascalCase symbols: ${exported.size}${!PROVIDER ? "" : exportEvidence ? `; bundle export list: ${exportEvidence.exports.size}` : " (bundle export evidence unavailable — scan fallback)"}`,
);
...
for (let p = PROVIDER; p; p = p.inner) {
  ...
  if (
    !/^[A-Za-z_$][\w$]*(\.[A-Za-z_$][\w$]*)*$/.test(String(p.component ?? ""))
  ) {
    console.error(
      `[PROVIDER_INVALID] cfg.provider component "${p.component}" isn't a valid identifier path (Name or Name.SubName) — fix cfg.provider.`,
    );
    process.exit(1);
  }
  ...
  if (exportEvidence) {
    ...
    if (!exportEvidence.cjsPresent && /^[A-Z][A-Za-z0-9]*$/.test(head)) {
      console.error(
        `[PROVIDER_UNEXPORTED] cfg.provider component "${p.component}" is not a bundle export (absent from the bundle's own export list) — every preview would fail with "Element type is invalid". Check the exact exported name, or export it via cfg.extraEntries.`,
      );
      process.exit(1);
    }
    console.error(
      `! [PROVIDER_UNVERIFIED] cfg.provider component "${p.component}" isn't in the bundle's export list (a bundled CJS module's re-exports can't be enumerated, or a non-PascalCase convention name) — proceeding on trust; if every preview fails with "Element type is invalid", the name is wrong.`,
    );
    continue;
  }
  ...
  if (/^[A-Z][A-Za-z0-9]*$/.test(head) && !exportScanLossy) {
    ...
    console.error(
      `[PROVIDER_UNEXPORTED] cfg.provider component "${p.component}" is not a bundle export — every preview would fail with "Element type is invalid". Check the exact exported name, or export it via cfg.extraEntries.`,
    );
    process.exit(1);
  }
  console.error(
    `! [PROVIDER_UNVERIFIED] cfg.provider component "${p.component}" isn't in the scanned export set (non-PascalCase name or a skipped export scan) — proceeding on trust; if every preview fails with "Element type is invalid", the name is wrong.`,
  );
}
...
for (const [el, names] of Object.entries(REPLACES_BY_ELEMENT)) {
  const n = (Array.isArray(names) ? names : [names]).find((c) =>
    exported.has(c),
  );
  if (n) REPLACES[n] = el;
}
...
if (!src.components.length && !src.tokensOnly) {
  console.error(
    `[ZERO_MATCH] ${shape === "storybook" ? "no story-type entries in storybook-static/index.json (only docs, or empty) — check the storybook config stories glob" : "no components discovered"}.`,
  );
  process.exit(1);
}
let components =
...
if (src.shape === "storybook") {
  ...
  console.error(
    `  ${components.length}/${src.components.length} storybook components are public exports`,
  );
}
// Dedup by name + sort.
const seen = new Set();
...
for (const n of dts.nonComponents) exported.delete(n);
{
  ...
  components = components.filter(
    (c) => !dts.nonComponents.has(c.name) && isComponentName(c.name),
  );
  console.error(
...
// name to be treated as a root.
{
  ...
  const { parentOf } = partitionSubcomponents(
  ...
  if (parentOf.size) {
    ...
    for (const [sub, parent] of parentOf)
      (byParent.get(parent) ?? byParent.set(parent, []).get(parent)).push(sub);
    for (const c of components)
      if (byParent.has(c.name)) c.subcomponents = byParent.get(c.name).sort();
    components = components.filter((c) => !parentOf.has(c.name));
    const sample = [...byParent]
      .slice(0, 3)
      .map(
        ([p, s]) =>
          `${p}←${s.slice(0, 3).join(",")}${s.length > 3 ? ",…" : ""}`,
      )
      .join("; ");
    console.error(
      `  (grouped ${parentOf.size} subcomponents under ${byParent.size} parents; ${components.length} roots: ${sample}${byParent.size > 3 ? "; …" : ""})`,
    );
  }
}
...
const guidelineFiles = emitGuidelines({
  cfg,
  PKG_DIR,
  OUT,
  cfgPath: wsCfgPath,
...
const uniformNamed = (() => {
  ...
  return components.length > 1 && only && only !== "general" && only !== "misc"
    ? only
    : null;
})();
...
for (const c of components) {
  ...
  if (
    ...
    if (g) {
      c.group = g;
      categoryApplied.add(c);
    }
  }
}
if (uniformNamed && categoryApplied.size > 0) {
  console.error(
    `  (single flat group "${uniformNamed}" across all components — doc frontmatter categories take precedence; doc-less components go to misc)`,
  );
...
writePreviewFiles({
  components,
  previewDir,
  genDir: genPreviewDir,
  gen: (c) =>
    generatePreviewSource(c, {
      exported,
      pkg: PKG,
...
emitPerComponent({
  ...
  propsBodyFor: (n) =>
    SKIP_DTS
      ? cfg.dtsPropsFor?.[n]
        ? {
            body: cfg.dtsPropsFor[n],
            generics: "",
            extendsClause: "",
            prelude: "",
          }
        : {
            body: "  [prop: string]: unknown; // stub — built with --skip-dts",
            generics: "",
            extendsClause: "",
            prelude: "",
          }
...
const { KEY_RECIPE, configSlicesFor, scriptsShaFor, sourceKeyFor } =
  await loadLib("sync-hashes");
...
const sourceKeys = Object.fromEntries(
  components.map((c) => [
    ...
    sourceKeyFor(c.name, {
      ...
      ...(shape === "storybook"
        ? { stories: c.visibleStoryIds ?? [], srcSha: c.srcSha ?? null }
        : {}),
    }),
  ]),
);
  ...
  JSON.stringify(
    {
      ...
      components: components.map((c) => ({
        ...
        stories: (c.visibleStoryIds ?? []).map((s) => ({
          id: s.id,
          name: s.name,
          exportKey: s.exportKey ?? null,
          emitted: s.emitted ?? null,
        })),
      })),
    },
    null,
...
writeStylesCss({
  ...
  bundleCss,
  fontRules,
  remoteImports: remoteStyleImports,
});
...
if (cfg.readmeHeader != null && CONFIG_PATH) {
  // cfg keys exist only when CONFIG_PATH was read; the guard keeps that invariant local instead of imported
  ...
  const cfgHome =
    basename(cfgDir) === ".design-sync" ? dirname(cfgDir) : cfgDir;
  ...
  if (!existsSync(cand)) {
    console.error(
      `  ! readmeHeader: ${cfg.readmeHeader} not found at the config home — skipped`,
    );
  } else if (outside(realpathSync(cand), headerRoot)) {
    console.error(
      `  ! readmeHeader: ${cfg.readmeHeader} resolves outside the config's repo — skipped`,
    );
  } else if (!statSync(cand).isFile()) {
    console.error(
      `  ! readmeHeader: ${cfg.readmeHeader} is not a regular file — skipped`,
    );
  } else if (
    statSync(cand).size <= 1_000_000 &&
    readFileSync(cand, "utf8").trim().length === 0
  ) {
    ...
    console.error(
      `  ! readmeHeader: ${cfg.readmeHeader} is ${statSync(cand).size} bytes — too large to be a prompt header, skipped`,
    );
  } else {
...
if (SKIP_DTS) {
  ...
  writeFileSync(
    metaPath,
    JSON.stringify(
      { ...JSON.parse(readFileSync(metaPath, "utf8")), dtsStubbed: true },
      null,
      2,
    ) + "\n",
  );
  console.error(
    "  [DTS_STUBBED] .d.ts bodies are stubs (--skip-dts) — validate will refuse this bundle for upload; run the final build without the flag",
  );
}
...
// exists.
{
  const { auxShaFor, styleShaFor, renderHashFor } =
    await loadLib("sync-hashes");
  const styleSha = styleShaFor(OUT, {
    includeBundleBody: shape !== "storybook",
  });
    ...
    JSON.stringify(
      {
        ...
        sourceHashes: headerMeta.sourceHashes,
  ...
  console.error(
    `  _ds_sync.json: ${components.length} render hash(es) + source key(s) (verification anchor)`,
  );
}
...
// after grading (a post-grade slim changes contracts and clears grades).
{
  ...
  const walk = (dir) => {
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (e.isFile() && statSync(p).size > 5 * 1024 * 1024)
        big.push([relative(OUT, p), statSync(p).size]);
    }
  };
  try {
    walk(OUT);
  ...
  for (const [p, sz] of big) {
    console.error(
      `! [FILE_OVER_5MB] ${p} is ${(sz / 1024 / 1024).toFixed(1)} MB — the upload rejects files over 5 MB. Slim it NOW (before grading): heavy dev-only deps (syntax highlighters, icons-as-code) usually don't belong in a preview or decorator bundle.`,
    );
  }
}

console.error(
  `✓ wrote ${OUT}: _ds_bundle.js + styles.css + ${count} component previews`,
);

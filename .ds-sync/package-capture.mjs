...
if (!OUT || !existsSync(join(OUT, ".stories-map.json"))) {
  console.error(
    "usage: package-capture.mjs --out <ds-bundle dir> (run package-build.mjs first)",
  );
  process.exit(2);
}
...
const all = (manifest.components ?? []).filter((c) =>
  existsSync(join(OUT, "_preview", `${c.name}.js`)),
);
const comps = ONLY
  ? all.filter((c) => ONLY.has(c.name) || SPOT_PICKS.includes(c.name))
  : all;
const floorCount = (manifest.components ?? []).length - all.length;
if (floorCount && !ONLY)
  console.error(
    `  (${floorCount} component(s) on the floor card — author previews to bring them into grading)`,
  );
const spotChecks = new Set(
  FORCE ? [] : SPOT_PICKS.filter((n) => all.some((c) => c.name === n)),
);
...
for (const n of SPOT_PICKS) {
  if (!spotChecks.has(n))
    console.error(
      `! --spot-check-components: "${n}" matches no capturable component (typo? renamed? floor card with no compiled preview?) — pick ignored`,
    );
}
if (spotChecks.size) {
  console.error(
    `◉ [SPOT_CHECK] re-verifying ${spotChecks.size} requested component(s): ${[...spotChecks].join(", ")} — grades kept; Read their fresh sheets and confirm they still match the recorded grades.`,
  );
}
...
try {
  ...
  console.error(
    "playwright not installed — npm i playwright (in .ds-sync/) first",
  );
  process.exit(2);
}
...
try {
  await page.clock.setFixedTime(new Date("2024-05-15T12:00:00Z"));
} catch {
  /* older playwright */
}
let pageErrs = [];
...
async function settle() {
  await page
    ...
    .catch(() => {});
  // Wait for framer-motion animations to complete. Staggered sequences (e.g.
  // HeroStatVisual bars: 13 × 45ms stagger + 400ms duration ≈ 985ms) need
  // more than the old 800ms. useHydratedReducedMotion also fires one RAF
  // before switching; capturing without reducedMotion:reduce lets animations
  // run to completion naturally rather than being cancelled mid-flight.
...
for (const c of comps) {
  ...
  let cardHead;
  try {
    cardHead = readFileSync(join(OUT, rel), "utf8").split("\n", 1)[0] ?? "";
  } catch {
    report.push({
      name: c.name,
      group: c.group,
      verdict: "error",
      reason: `missing ${rel} — rebuild (package-build.mjs) before capturing`,
    });
    console.error(
      `✗ [CAPTURE] ${c.name}: missing ${rel} — rebuild (package-build.mjs) before capturing`,
    );
    continue;
  }
  ...
  let prev = null,
    grade = null;
  try {
    prev = JSON.parse(readFileSync(capPath, "utf8"));
  } catch {
    /* first capture */
  }
  try {
  ...
  if (
    c.sourceKey &&
    prev &&
    prev.gradeKey !== gradeKey &&
    (prev.keyRecipe ?? 0) !== KEY_RECIPE &&
  ...
  try {
    ...
    cells = await page.evaluate(() =>
      Array.isArray(window.__dsCells) ? window.__dsCells.slice() : [],
    );
  } catch (e) {
    report.push({
      name: c.name,
    ...
    console.error(
      `✗ [CAPTURE] ${c.name}: ${String(e.message ?? e).split("\n")[0]}`,
    );
    continue;
  }
  if (cells.length === 0) {
    ...
    const why =
      pageErrs[0] ??
      "preview module evaluated to no exports (window.__dsCells is empty)";
    report.push({
      name: c.name,
      group: c.group,
      verdict: "error",
    ...
    console.error(
      `✗ [CAPTURE] ${c.name}: ${why} — fix the preview (.design-sync/previews/${c.name}.tsx) and rebuild`,
    );
    continue;
  }
  ...
  if (
    ...
    if (prev.pendingGrade !== false) {
      writeFileSync(
        capPath,
        JSON.stringify({ ...prev, pendingGrade: false }, null, 2),
      );
    }
  ...
  for (const label of cells) {
    try {
      await page.goto(
        `http://127.0.0.1:${port}/${rel}?story=${encodeURIComponent(label)}`,
    ...
    shots.push({
      label,
      png: png ? `raw/${file}` : null,
      err: info.caught ? info.text.slice(0, 120) : null,
    });
  }
  ...
  const rows = shots
    .map(
      (s) =>
        ...
        `<td style="vertical-align:top;padding:8px;border-left:1px solid #eee">${s.png ? `<img src="./${s.png}" style="max-width:760px;max-height:520px;display:block">` : '<div style="color:#999">(no shot)</div>'}</td></tr>`,
    )
    .join("\n");
  const sheetHtml =
    `<!doctype html><html><head><meta charset="utf-8"></head><body style="margin:0;background:#fff;width:980px;font-family:system-ui">` +
  ...
  try {
    ...
    await page.goto(
      `http://127.0.0.1:${port}/_screenshots/review/.sheet-${c.group}__${c.name}.html`,
    ...
    await page.screenshot({
      path: join(shotBase, `${c.group}__${c.name}.png`),
      fullPage: true,
    });
  } catch (e) {
    console.error(
  ...
  const pendingGrade = !(
    cells.length > 0 &&
    cells.every((k) => grade?.cells?.[k]?.verdict === "good")
  );
  writeFileSync(
    capPath,
    JSON.stringify(
      {
        ...
        cells,
        pendingGrade,
        shots: shots.map((s) => s.label),
        pageErrs: [...new Set(pageErrs)].slice(0, 3),
      },
      null,
      2,
    ),
  ...
  const keyHint = cells.length
    ? ` — grade keys: ${cells.map((k) => JSON.stringify(k)).join(", ")}`
    : "";
  console.error(
    `${errCells ? "✗" : "○"} [CAPTURE] ${c.name}: ${cells.length} cell(s)${errCells ? `, ${errCells} error(s)` : " need grading"}${keyHint}`,
  );
}
...
if (!ONLY) {
  ...
  // can't silently ship.
  try {
    const unmerged = readdirSync(resolve(".design-sync", "learnings")).filter(
      (f) => f.endsWith(".md"),
    );
    if (unmerged.length)
      console.error(
        `! [LEARNINGS_UNMERGED] ${unmerged.length} file(s) in .design-sync/learnings/ — fold into NOTES.md and delete them before upload`,
      );
  } catch {
    /* no learnings dir */
  }
...
console.error(
  "Grade from the sheets: Read each _screenshots/review/<group>__<Name>.png, then Write verdicts to .design-sync/.cache/review/<Name>.grade.json (keys must equal the cell labels exactly).",
);

...
export async function probe({ sbStatic, firstStoryId, exportedNames = [] }) {
  ...
  try {
    ...
    await page
      ...
      .catch(() => {});
    await page
      .evaluate(`(${FIBER_WALK_STORE})(${JSON.stringify(exportedNames)})`)
      .catch(() => 0);
    ...
    if (provider)
      console.error(
        `[PROVIDER_DETECTED] ${chain.map((p) => p.component).join(" > ")}`,
      );
    return { provider };
  } catch (e) {
...
if (
  ...
  const sbStatic =
    flag("storybook-static") && resolve(flag("storybook-static"));

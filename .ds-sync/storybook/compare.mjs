...
// Unconsumed argv is silently dead otherwise — `--components A B` runs only A.
{
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    console.error(
      `(unrecognized argument "${argv[i]}" — ignored; multi-component scoping is comma-separated: --components A,B)`,
    );
  }
}
const OUT = flag("out") && resolve(flag("out"));
const ONLY = flag("components")
  ? new Set(
      flag("components")
        .split(",")
        .map((s) => s.trim())
  ...
  : null;
const SPOT_PICKS = flag("spot-check-components")
  ? flag("spot-check-components")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
  : [];
...
if (!OUT || !existsSync(join(OUT, ".stories-map.json"))) {
  console.error(
    "usage: node storybook/compare.mjs --out <ds-bundle> --storybook-static <dir> [--components A,B]",
  );
  console.error(
    "(requires <out>/.stories-map.json — run package-build.mjs first)",
  );
  process.exit(2);
}
...
if (manifest.keyRecipe !== KEY_RECIPE)
  for (const c of manifest.components ?? []) delete c.sourceKey;
const SB = resolve(flag("storybook-static", manifest.storybookStatic ?? ""));
if (!SB || !existsSync(join(SB, "iframe.html"))) {
  console.error(
    `[SB_REFERENCE_MISSING] ${SB || "(unset)"} has no iframe.html — build the reference storybook first (npx storybook build -o .design-sync/sb-reference) and pass --storybook-static.`,
  );
  process.exit(2);
}

const comps = manifest.components.filter(
  (c) =>
    c.stories.length &&
    (!ONLY || ONLY.has(c.name) || SPOT_PICKS.includes(c.name)),
);
...
if (ONLY && !comps.some((c) => ONLY.has(c.name))) {
  console.error(
    `[ZERO_MATCH] none of ${[...ONLY].join(", ")} have stories in .stories-map.json`,
  );
  process.exit(2);
}
if (!comps.length) {
  console.error(
    ONLY
      ? `[ZERO_MATCH] none of ${[...ONLY].join(", ")} have stories in .stories-map.json`
      : "[ZERO_MATCH] no components with stories — compare needs the storybook shape",
  );
  process.exit(2);
}
...
try {
  ...
  console.error(
    "[NO_CHROMIUM] compare requires playwright — npm i playwright && npx playwright install chromium",
  );
  process.exit(2);
}
...
const oldGradeKeyFor = (c) =>
  gradeKeyFrom(
    renderHashFor(OUT, c, { stories: c.stories, srcSha: c.srcSha ?? null }),
  );
const gradeKeyFor = (c) =>
  c.sourceKey ? gradeKeyFrom(c.sourceKey) : oldGradeKeyFor(c);
...
hashDir(
  ...
  OUT,
  "out/",
  new Set(["_screenshots", "_preview", "components", "_ds_sync.json"]),
);
...
for (const c of comps) {
  ...
  let grade = null;
  try {
    prevCapture = JSON.parse(
      readFileSync(join(cacheDir, `${c.name}.json`), "utf8"),
    );
  } catch {
    /* first capture */
  }
  try {
    grade = JSON.parse(
      readFileSync(join(cacheDir, `${c.name}.grade.json`), "utf8"),
    );
  } catch {
    /* ungraded */
  }
  ...
  if (
    c.sourceKey &&
    prevCapture &&
    prevCapture.gradeKey !== gradeKey &&
    (prevCapture.keyRecipe ?? 0) !== KEY_RECIPE
  ) {
    const adopt = () => {
      prevCapture = {
        ...prevCapture,
      ...
      writeFileSync(
        join(cacheDir, `${c.name}.json`),
        JSON.stringify(prevCapture, null, 2),
      );
    };
    if (prevCapture.gradeKey === oldGradeKeyFor(c)) {
      ...
      prevCapture.srcSha != null &&
      c.srcSha != null &&
      prevCapture.srcSha === c.srcSha &&
  ...
  if (grade?.stories) {
    ...
    if (unknown.length) {
      console.error(
        `  (grade key(s) matching no story for ${c.name}: ${unknown.slice(0, 4).join(", ")} — keys must equal story names exactly, e.g. ${JSON.stringify(c.stories[0]?.name ?? "")})`,
      );
    }
  }
  ...
  const fullyGraded =
    ...
    storyNames.length > 0 &&
    storyNames.every((n) =>
      ["match", "close"].includes(grade.stories[n]?.verdict),
    );
...
if (FORCE && SPOT_PICKS.length) {
  console.error(
    "  (--force demands fresh verdicts everywhere — --spot-check-components picks are captured and re-graded like everything else)",
  );
}
for (const name of SPOT_PICKS) {
  ...
  if (!p) {
    console.error(
      `(spot-check pick "${name}" has no stories in .stories-map.json — ignored)`,
    );
    continue;
  }
  if (FORCE) continue;
  if (p.fullyGraded && p.prevCapture?.gradeKey === p.gradeKey)
    spotChecks.add(name);
  else
    console.error(
      `  (spot-check pick ${name} is not a fully-graded carried-forward component — captured under the normal rules instead)`,
    );
}
if (spotChecks.size) {
  console.error(
    `◉ [SPOT_CHECK] re-verifying ${spotChecks.size} requested carried-forward component(s): ${[...spotChecks].join(", ")} — grades kept; Read their fresh sheets and confirm they still match the recorded grades. Divergence remediation scales with the churned set: a couple of components diverge — re-grade just those; widespread — stop, diagnose, then --force a full pass.`,
  );
}
if (!ONLY && !FORCE && SPOT_CHECK_N > 0) {
  const candidates = comps.filter((c) => {
    ...
    return (
      !spotChecks.has(c.name) &&
      p.fullyGraded &&
      p.prevCapture?.gradeKey === p.gradeKey &&
      (p.prevCapture.sbBaseSha !== SB_BASE_SHA ||
        p.prevCapture.outBaseSha !== OUT_BASE_SHA)
    );
  });
  ...
  if (sampled.length) {
    console.error(
      `◉ [SPOT_CHECK] re-verifying ${sampled.length} carried-forward component(s) after shared-input changes: ${sampled.join(", ")} — Read their fresh sheets and confirm they still match the recorded grades. Divergence remediation scales with the churned set: a couple of components diverge — re-grade just those; widespread — stop, diagnose, then --force a full pass.`,
    );
  }
}
...
if (migrationPool.length && !FORCE) {
  ...
  const picks = new Set(
    eligible.filter((n) => pre.get(n).prevCapture?.portal).slice(0, 1),
  );
  ...
  console.error(
    `◉ [SPOT_CHECK] grade-key migration: adopted ${migrationPool.length} component(s) whose artifacts churned while their sources held${picks.size ? `; re-verifying ${picks.size} of them: ${[...picks].join(", ")} — grades kept; Read their fresh sheets and confirm they still match the recorded grades` : ""}.`,
  );
}
...
try {
  ...
  const dsPage = await browser.newPage({
    viewport: { width: 900, height: 700 },
  });
  ...
  for (const p of [sbPage, dsPage]) {
    ...
    await p.clock
      ?.setFixedTime(new Date("2030-01-15T12:00:00Z"))
      .catch(() => {});
  }
  ...
  async function captureStory(id) {
    try {
      await sbPage.goto(
        `http://127.0.0.1:${sbPort}/iframe.html?id=${encodeURIComponent(id)}&viewMode=story`,
        { waitUntil: "networkidle", timeout: 20_000 },
      );
    } catch {
      /* fall through to the selector wait — slow asset ≠ broken story */
    }
    const loaded = await sbPage
      .waitForSelector(SB_CONTENT, { timeout: 8_000 })
      .then(() => true)
      .catch(() => false);
    if (!loaded) {
      // .sb-errordisplay is always present as a display:none template — only
      // report its text when it's actually visible.
      const err = await sbPage
        .evaluate(() => {
          const e = document.querySelector(".sb-errordisplay");
          return e && getComputedStyle(e).display !== "none"
            ? e.textContent?.slice(0, 160)
            : "no storybook root content";
        })
        .catch(() => "?");
      return { err };
    }
    ...
    try {
      ...
      /* full-page fallback below */
    }
    if (!png || png.length < 200) {
      try {
        png = await sbPage.screenshot({ ...SHOT, fullPage: false });
      } catch {
        /* keep null */
      }
    }
  ...
  for (const c of comps) {
    ...
    if (
      ...
      if (prevCapture.pendingGrade !== false) {
        writeFileSync(
          capturePath,
          JSON.stringify({ ...prevCapture, pendingGrade: false }, null, 2),
        );
        prevCapture.pendingGrade = false;
      }
      report.push({ ...prevCapture, skipped: true });
      console.error(
        `↻ [COMPARE] ${c.name}: sources unchanged & fully graded — carried forward (--force to recapture)`,
      );
      continue;
    }
    ...
    if (
      !warnedStaleRef &&
      prevCapture &&
      prevCapture.sbBaseSha === SB_BASE_SHA &&
      prevCapture.outBaseSha !== OUT_BASE_SHA
    ) {
      warnedStaleRef = true;
      console.error(
        "! [REFERENCE_STALE?] the bundle changed but .design-sync/sb-reference did not — if the DS source changed, rebuild the reference first (a stale reference makes compare grade against the OLD design)",
      );
    }
    ...
    if (!existsSync(join(OUT, rel))) {
      report.push({
        ...
        group: c.group,
        verdict: "error",
        reason: `${rel} missing — run package-build.mjs`,
      });
    ...
    if (grade && (FORCE || prevCapture?.gradeKey !== gradeKey)) {
      ...
      grade = null;
      console.error(
        `  (grade cleared for ${c.name} — ${FORCE ? "--force requested fresh verdicts" : "contract changed"}; re-grade from the fresh sheet)`,
      );
    }
    dsErrs = [];
    let pageErr = null;
    ...
    // config plumbing.
    const vpMatch = /viewport="(\d+)x(\d+)"/.exec(
      readFileSync(join(OUT, rel), "utf8").split("\n", 1)[0] ?? "",
    );
    const vp = vpMatch
      ? {
          width: Math.min(+vpMatch[1], 2000),
          height: Math.min(+vpMatch[2], 2000),
        }
      : { width: 900, height: 700 };
    await sbPage.setViewportSize(vp);
    await dsPage.setViewportSize(vp);
    try {
      await dsPage.goto(`http://127.0.0.1:${outPort}/${rel}`, {
      ...
      if (/Timeout/i.test(String(e.message ?? e)))
        console.error(
          `  (networkidle timeout on ${c.name} — capturing after settle)`,
        );
    ...
    const pv = pageErr
      ...
      : await dsPage
          .evaluate(() => {
            const kind = document.querySelector('script[src*="_preview/"]')
              ? "module"
              : "fallback";
            ...
            const dsCells = Array.isArray(window.__dsCells)
              ? window.__dsCells.slice()
              : null;
            const cells = dsCells
              ? dsCells.map((label, i) => ({ i, label }))
              : [...document.querySelectorAll("section.ds-cell")].map(
                  (s, i) => {
            ...
            const portal = [...document.body.children].some(
              (el) =>
                !el.matches(
                  ".ds-grid, .ds-single, section, script, style, link, h4, div[id]",
                ) &&
            ...
            return {
              ...
              portal,
              perStory: !!dsCells,
              mode: window.__dsMode ?? "grid",
            };
          })
          .catch((e) => {
            pageErr = String(e).split("\n")[0];
            return null;
          });
    ...
    if (pageErr || !pv) {
      report.push({
        ...
        group: c.group,
        verdict: "error",
        reason: `preview page failed: ${pageErr}`,
      });
    ...
    if (c.stories.length > MAX_STORIES) {
      console.error(
        `  [STORY_CAP] ${c.name}: comparing first ${MAX_STORIES} of ${c.stories.length} stories — pass --max-stories ${c.stories.length} to cover all`,
      );
    }
    const cellByLabel = new Map(
      pv.cells.map((cell) => [squash(cell.label), cell]),
    );
    ...
    const pairs = stories.map((s) => {
      ...
      if (cell && !usedCells.has(cell.i)) {
        usedCells.add(cell.i);
        return { story: s, cell, pairedBy: "name" };
      }
    ...
    const overCap = new Set(
      c.stories
        .slice(MAX_STORIES)
        .map((s) => squash(s.emitted ?? s.exportKey ?? exportName(s.name))),
    );
    const extraCells = pv.cells
      .filter(
        (cell) => !usedCells.has(cell.i) && !overCap.has(squash(cell.label)),
      )
      .map((cell) => cell.label);
    if (extraCells.length) {
      ...
      console.error(
        `  (extra cells not matching any story for ${c.name}: ${extraCells.join(", ")})`,
      );
    }
    ...
    if (pv.portal && pv.mode !== "single") {
      console.error(
        `  [PORTAL?] ${c.name}: overlay/portal content renders outside its cells — consider cfg.overrides ${c.name}: {"cardMode": "single"}`,
      );
    }
    ...
    async function cellShot(cell) {
      ...
      try {
        if (mount && cell.w >= 4 && cell.h >= 4)
          return await mount.screenshot(SHOT);
    ...
    async function storyShot(label) {
      try {
        await dsPage.goto(
          `http://127.0.0.1:${outPort}/${rel}?story=${encodeURIComponent(label)}`,
          { waitUntil: "networkidle", timeout: 20_000 },
        );
      } catch (e) {
      ...
      const png = await dsPage
        .screenshot({ ...SHOT, fullPage: false })
        .catch(() => null);
      return { png, ...info };
    }
    ...
    for (const p of pairs) {
      ...
      if (!p.cell) {
        storyResults.push({
          ...
          reasons: [
            pv.kind === "fallback"
              ? "preview is the floor card (no compiled preview) — author this story in .design-sync/previews/" +
                c.name +
                ".tsx"
    ...
    if (dsErrs.length)
      storyResults.push({
        story: "(page)",
        verdict: "error",
        reasons: [...new Set(dsErrs)].slice(0, 3),
      });
    ...
    const srcChanged = !!(
      prevCapture &&
      prevCapture.srcSha !== undefined &&
      (prevCapture.srcSha ?? null) !== (c.srcSha ?? null) &&
      (prevCapture.srcSha || c.srcSha)
    );
    ...
    // here.)
    const ownedPreview = existsSync(
      resolve(".design-sync", "previews", `${c.name}.tsx`),
    );
    const storyChanged = storyResults
      .filter((r) => r.storyChanged)
      .map((r) => r.story);
    if (storyChanged.length) {
      console.error(
        `! [STORY_CHANGED] ${c.name}: ${storyChanged.join(", ")} — the story itself changed${
          ownedPreview
    ...
    const rows = storyResults
      .map((r) => {
        const base = `${c.group}__${c.name}__${squash(r.id ?? r.story) || "story"}`;
        const img = (suffix) =>
          existsSync(join(rawDir, `${base}__${suffix}.png`))
        ...
        return (
          `<tr><td style="vertical-align:top;padding:8px;font:600 14px system-ui">${escapeHtml(r.story)}<br><span style="color:${color}">${r.verdict}</span><br><span style="font-weight:400;font-size:12px;color:#555">${(r.reasons ?? []).map(escapeHtml).join("<br>")}</span></td>` +
      ...
      .join("\n");
    const sheetHtml =
      `<!doctype html><html><head><meta charset="utf-8"></head><body style="margin:0;background:#fff;width:1180px;font-family:system-ui">` +
    ...
    try {
      await dsPage.goto(
        `http://127.0.0.1:${outPort}/_screenshots/compare/.sheet-${c.group}__${c.name}.html`,
      ...
      await dsPage.screenshot({
        path: join(cmpDir, `${c.group}__${c.name}.png`),
        fullPage: true,
      });
    } catch (e) {
      console.error(
        `  (sheet skipped for ${c.name} — ${String(e).split("\n")[0]})`,
      );
    }
    ...
    const pendingGrade = !(
      gradable.length > 0 &&
      gradable.every((r) =>
        ["match", "close"].includes(grade?.stories?.[r.story]?.verdict),
      )
    );
    const entry = {
      name: c.name,
      ...
      keyRecipe: c.sourceKey ? KEY_RECIPE : undefined,
      sbBaseSha: SB_BASE_SHA,
      outBaseSha: OUT_BASE_SHA,
      gradeKey,
      counts,
      extraCells,
    ...
    const summary = Object.entries(counts)
      ...
      .join(", ");
    const mark =
      counts.error || counts.unpaired || counts["sb-error"] ? "✗" : "○";
    ...
    const keyHint = counts["needs-grade"]
      ? ` — grade keys: ${storyResults
          .filter((r) => r.verdict === "needs-grade")
          .map((r) => JSON.stringify(r.story))
          .join(", ")}`
      : "";
    console.error(
      `${mark} [COMPARE] ${c.name}: ${summary}${pv.kind === "fallback" ? " (fallback preview)" : ""}${keyHint}`,
    );
    // Printed only when a signature matches — never a hint without its
    // corroborating error.
    if (counts.error) {
      const firstErr = storyResults.find((r) => r.verdict === "error")
        ?.reasons?.[0];
...
const hard = report.filter(
  (r) =>
    !r.skipped &&
    (r.verdict === "error" ||
      (r.counts &&
        (r.counts.error || r.counts.unpaired || r.counts["sb-error"]))),
);
if (!ONLY) {
  ...
  const withGrades = report.map((r) => {
    ...
    try {
      return {
        ...r,
        grades: JSON.parse(
          readFileSync(join(cacheDir, `${r.name}.grade.json`), "utf8"),
        ),
      };
    } catch {
      return { ...r, grades: null };
    }
  });
  writeFileSync(
    join(OUT, ".compare-report.json"),
    JSON.stringify({ components: withGrades }, null, 2) + "\n",
  );
  ...
  try {
    const pendingLearnings = readdirSync(
      resolve(".design-sync", "learnings"),
    ).filter((f) => f.endsWith(".md"));
    if (pendingLearnings.length) {
      console.error(
        `[LEARNINGS_UNMERGED] ${pendingLearnings.length} file(s) in .design-sync/learnings/ — promote [GENERAL] bullets into .design-sync/NOTES.md, then delete the folder. Do not upload while this prints.`,
      );
    }
  } catch {
    /* no learnings dir — nothing pending */
  }
}
if (blockedHosts.size) {
  console.error(
    `! [ASSETS_BLOCKED] external assets failed to load during capture: ${[...blockedHosts].slice(0, 8).join(", ")}${blockedHosts.size > 8 ? ", …" : ""}. If this shell sandboxes network egress, BOTH panels rendered without these assets and grades can falsely pass while claude.ai/design users see different output. Re-run package-validate.mjs and compare.mjs --force from a shell with egress to these hosts (approve running the command without the sandbox when prompted, or add the hosts to the sandbox allowlist).`,
  );
}
const skipped = report.filter((r) => r.skipped);
const pending = report.filter(
  (r) => !r.skipped && r.counts?.["needs-grade"] && !hard.includes(r),
);
console.error(
...
console.error(
  "Grade from the true images: Read each _screenshots/compare/<group>__<Name>.png sheet (raw/ PNGs are the full-res authority), then Write the verdicts to .design-sync/.cache/compare/<Name>.grade.json (a recapture clears the old grade — its contract changed).",
);

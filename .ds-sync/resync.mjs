...
if (!existsSync(resolve(".design-sync"))) {
  console.error(
    `✗ no .design-sync/ under ${process.cwd()} — run the driver from the repo root`,
  );
  process.exit(2);
}
...
// staying a stage failure.
{
  ...
  if (parsed) {
    ...
    if (errs.length) {
      ...
      console.error(
        `✗ ${CONFIG}: fix the config and re-run — nothing was built`,
      );
      process.exit(2);
    }
  }
...
function runStage(name, script, args) {
  ...
  if (exit !== 0 && firstFailExit === null)
    firstFailExit = exit === 2 ? 1 : exit;
  return exit === 0;
}
...
// run's diff.
const syncDiff = stages.diff?.ok
  ? readJson(join(OUT, ".sync-diff.json"))
  : null;
...
if (aborted) {
  ...
  const args = [OUT];
  if (flag("render-sample"))
    args.push("--render-sample", flag("render-sample"));
  if (argv.includes("--no-render-check")) args.push("--no-render-check");
  const userScoped =
    argv.includes("--render-sample") || argv.includes("--no-render-check");
  ...
  if (
    ...
    !(
      syncDiff.changed?.length ||
      syncDiff.added?.length ||
      syncDiff.renderChurned?.length
    )
  ) {
...
if (aborted) {
  ...
  aborted = true;
  console.error(
    "✗ .sync-diff.json unreadable after a clean diff stage — cannot scope the capture",
  );
} else {
  worklist = [...(syncDiff.changed ?? []), ...(syncDiff.added ?? [])];
  const manifest =
    shape === "storybook" ? readJson(join(OUT, ".stories-map.json")) : null;
  ...
  if (worklist.length && isCapturable) {
    ...
    // members still re-ship via the upload partition.
    {
      ...
      const broken =
        shape !== "storybook"
          ? dropped.filter((n) =>
              existsSync(resolve(".design-sync", "previews", `${n}.tsx`)),
            )
          : [];
  ...
  let refDrift = false;
  {
    const churned = (syncDiff.renderChurned ?? []).filter((n) =>
      isCapturable?.(n),
    );
    if (shape === "storybook") {
    ...
    const pool = [
      ...new Set([
        ...churned,
        ...(refDrift
          ? (syncDiff.unchanged ?? []).filter((n) => isCapturable?.(n))
          : []),
      ]),
    ...
    if (pool.length) {
      ...
      const capOf = (n) => {
        if (!capJson.has(n))
          capJson.set(n, readJson(join(gradeCacheDir, `${n}.json`)));
      ...
      const single = (n) =>
        ["single", "column"].includes(cfgObj.overrides?.[n]?.cardMode) ||
        capOf(n)?.portal === true;
      const picks = new Set();
  ...
  if (!worklist.length && !canaryPicks.length) {
    ...
    console.error(
      "capture skipped — no capturable changed or added components",
    );
  } else {
    ...
    if (canaryPicks.length)
      args.push("--spot-check-components", canaryPicks.join(","));
    if (shape === "storybook") {
  ...
  ...new Set([
    ...(stages.capture.skipped !== null
      ...
      : stages.capture.ok
        ? scanSet.filter(
            (name) =>
              readJson(join(gradeCacheDir, `${name}.json`))?.pendingGrade !==
              false,
          )
        : scanSet),
...
const ok =
  ...
  Object.values(stages).every((s) =>
    s.skipped === null ? s.ok : s.skipped !== "prior_failure",
  );
const verdict = {
  version: 2,
  ok,

...
function StationFacet({
  ...
  return (
    <span className="relative block">
      ...
      {/* Clean rounded card — mirrors the detail container on the right. */}
      <span
        className={cn(
          "cta-panel-frame relative flex items-center gap-3.5 rounded-2xl border p-5 backdrop-blur-sm transition-[border-color,background-color] duration-[250ms]",
          isActive ? "border-brand bg-card/85" : "border-border bg-card/60",
        )}
      >
        <span
          className={cn(
            "flex size-11 shrink-0 items-center justify-center rounded-lg border transition-colors duration-[250ms]",
            isActive
        ...
        <span className="flex flex-col">
          ...
          <span
            className={cn(
              "text-base leading-tight font-semibold transition-colors duration-[250ms]",

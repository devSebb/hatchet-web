...
//
// Deliberately left on the monogram fallback:
//   * Kakao (KakaoTV) -- simple-icons carries it, but it is a wordmark that
//     collapses into an illegible smudge at 22px. Same call as Rooter above: a
//     mark that does not survive the grid size loses to a clean letter.
//   * MLG -- tried both framings: the parallelogram with the figure knocked out
//     fragments into disconnected shards, and the figure alone is an unreadable
//     blob. The brand is also absorbed into Blizzard now, with no live logo source.
//   * Loco, Mildom -- no single-colour mark found in any open library, on
//     Wikimedia Commons (wordmark-only), or as a pinned-tab mask on the brand's
//     own site. Favicons are 16-32px, too coarse to trace. Both are archived, where
//     a muted monogram fits the greyed treatment. Either can be added later by
//     handing the tracer a flat, geometric source -- the constraint is the mark,
//     not the file format or the resolution.
